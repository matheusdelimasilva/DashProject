# ─── Router ──────────────────────────────────────────────────────────────────
# URL pattern matching with static and dynamic route segments.
...
struct DynamicRoute{ST,VT} <: AbstractRoute
    ...
    DynamicRoute(segments_length, static_segments::ST, variables::VT, tailed) where {ST,VT} =
        new{ST,VT}(segments_length, static_segments, variables, tailed)
end
...
function DynamicRoute(url::AbstractString)
    ...
    for part in parts
        ...
        if length(part) > 2 && part[1] == '<' && part[end] == '>'
            ...
            var_name in keys(variables) && throw(ArgumentError("duplicated variable name $(var_name)"))
...
function try_handle(route_handler::RouteHandler{StaticRoute}, path::AbstractString, request::HTTP.Request, args...)
...
function try_handle(route_handler::RouteHandler{<:DynamicRoute}, path::AbstractString, request::HTTP.Request, args...)
...
function Route(handler::Function, method, path::AbstractString)
    return Route(method, RouteHandler(_make_route(path), handler))
end
...
@inline function _handle(route_tuple::Tuple, path::AbstractString, request::HTTP.Request, args...)
    ...
    return !isnothing(res) ? res : _handle(Base.tail(route_tuple), path, request, args...)
end

@inline function _handle(::Tuple{}, path::AbstractString, request::HTTP.Request, args...)
